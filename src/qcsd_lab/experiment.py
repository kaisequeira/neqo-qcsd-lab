...
def validate_accepted_scheduler_runtime_receipt(
    ...
    if (
        ...
        and runner_schema not in {10, 15, 16}
        and not _historical_buflo_v36_experiment(experiment)
    ):
        raise ValueError(
            "current BuFLO-study sample requires runner-wakeup schema 10/15/16 or an exact "
    ...
    if current_class_role and runtime_kind == "buflo":
        if runner_schema != 16:
            raise ValueError(
                "current class-study BuFLO sample requires runner-wakeup schema 16 "
                "with kernel-TX evidence"
            )
    elif current_buflo_role and runtime_kind == "buflo":
        if runner_schema not in {15, 16}:
            raise ValueError(
                "BuFLO-study sample requires runner-wakeup schema 15/16 "
                "with kernel-TX evidence"
            )
    ...
    elif current_class_role and runner_schema not in {10, 16}:
        raise ValueError("current class-study sample requires runner-wakeup schema 10/16")
