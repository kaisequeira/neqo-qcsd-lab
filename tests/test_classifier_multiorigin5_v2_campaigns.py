...
QUALIFICATION_SET = "classifier-multiorigin5-v2"
WORKLOADS = (
...
BASELINE_SEEDS = tuple(range(2_026_082_101, 2_026_082_111))
# Prospectively selected from 2026122001..2026222000 using deterministic plan
...
EXPECTED_PAIRED_POSITION_COUNTS = (
    ...
    ((34, 32, 34), (33, 33, 34), (33, 35, 32)),
    ((34, 33, 33), (33, 33, 34), (33, 34, 33)),
    ((34, 33, 33), (33, 34, 33), (33, 33, 34)),
)
FORMAL_PLAN_SHA256 = "dd7f81558ddcd48853aec6287548f29759212329738e42b9d77b1c0a1dd84d47"
REHEARSAL_PLAN_SHA256 = "57a9a1a5df318bbf392c098051b972a49b4e4dc09ab410d2eb56b4ce815d44a6"
ALL_PLAN_SHA256 = "7024d76aa4acd7a02ae45a3febd364ee06f0db5bc92a3276b42c40a9777dd2f9"
LIMITS = {
    "timeout_seconds": 120,
...
FORMAL_CASES = tuple(
    (
        block,
        kind,
        CAMPAIGN_DIR / f"classifier-multiorigin5-v2-{kind}-{block:02d}.yml",
...
REHEARSAL_PATH = CAMPAIGN_DIR / "classifier-multiorigin5-v2-rehearsal.yml"
V1_PATHS = tuple(
    CAMPAIGN_DIR / f"classifier-multiorigin5-v1-{kind}-{block:02d}.yml"
    for block in range(1, 11)
    for kind in ("baseline", "paired")
) + (CAMPAIGN_DIR / "classifier-multiorigin5-v1-rehearsal.yml",)
POC5_PATHS = tuple(
...
def _plan_sha256(samples: tuple[dict[str, Any], ...]) -> str:
...
def test_multiorigin5_v2_formal_campaign_contract(
    ...
    assert value["name"] == (f"research-classifier-multiorigin5-v2-{kind}-{block:02d}-1200")
...
def test_multiorigin5_v2_workloads_freeze_complete_rendered_get_coverage(
    planning_base: Campaign,
) -> None:
...
def test_multiorigin5_v2_defended_campaigns_bind_one_exact_qualification_set() -> None:
...
def test_multiorigin5_v2_formal_aggregate_is_exactly_2500_balanced_samples(
    planning_base: Campaign,
) -> None:
    actual_paths = tuple(
        sorted(CAMPAIGN_DIR.glob("classifier-multiorigin5-v2-baseline-*.yml"))
    ) + tuple(sorted(CAMPAIGN_DIR.glob("classifier-multiorigin5-v2-paired-*.yml")))
    ...
    assert _plan_sha256(combined) == FORMAL_PLAN_SHA256
    ...
    assert sum((3 * count - 100) ** 2 for count in defense_positions.values()) == 144
...
def test_multiorigin5_v2_chronology_contract_alternates_within_blocks() -> None:
    capture_order = tuple(
        path
        for block in range(10)
        for path in (
...
def test_multiorigin5_v2_rehearsal_is_excluded_and_fresh_from_v1_and_historical_poc5(
    planning_base: Campaign,
    poc5_base: Campaign,
) -> None:
    value = _load_value(REHEARSAL_PATH)
    assert set(value) == BASE_KEYS | {"chaff_qualification_set"}
    assert value["name"] == "research-classifier-multiorigin5-v2-rehearsal-1200"
    ...
    assert set(CAMPAIGN_DIR.glob("classifier-multiorigin5-v2-*.yml")) == {
        *FORMAL_PATHS,
        REHEARSAL_PATH,
    }

    current = _planned(planning_base, (*FORMAL_PATHS, REHEARSAL_PATH))
    v1 = _planned(planning_base, V1_PATHS)
    poc5 = _planned(poc5_base, POC5_PATHS)
    ...
    assert _plan_sha256(current) == ALL_PLAN_SHA256
    assert _plan_sha256(rehearsal) == REHEARSAL_PLAN_SHA256
    for historical in (v1, poc5):
        assert len(historical) == 2_530
    ...
    lineage_seeds = {*BASELINE_SEEDS, *PAIRED_SEEDS, REHEARSAL_SEED}
    lineage_paths = {*FORMAL_PATHS, REHEARSAL_PATH}
    historical_campaign_seeds = {
        campaign["seed"]
        for path in CAMPAIGN_DIR.glob("*.y*ml")
        if path not in lineage_paths
        if isinstance((campaign := _load_value(path)).get("seed"), int)
    }
    assert len(lineage_seeds) == 21
    assert lineage_seeds.isdisjoint(historical_campaign_seeds)
    for samples in (current, v1, poc5):
        assert lineage_seeds.isdisjoint({sample["seed"] for sample in samples})
