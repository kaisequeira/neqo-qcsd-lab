...
PILOT_PATHS = tuple(
    CAMPAIGN_DIR / f"classifier-stable3-pilot-{block:02d}.yml" for block in range(1, 8)
)
WORKLOADS = (
...
WORKLOAD_ORDERS = (
    WORKLOADS,
    (WORKLOADS[2], WORKLOADS[0], WORKLOADS[1]),
    (WORKLOADS[1], WORKLOADS[0], WORKLOADS[2]),
    (WORKLOADS[0], WORKLOADS[2], WORKLOADS[1]),
    (WORKLOADS[2], WORKLOADS[1], WORKLOADS[0]),
    (WORKLOADS[1], WORKLOADS[2], WORKLOADS[0]),
    WORKLOADS,
)
DEFENSES = (
    "undefended",
...
def test_stable3_pilot_block_contract_and_exact_21_sample_expansion(
    ...
    assert value["name"] == f"research-classifier-stable3-pilot-{block:02d}-1200"
    ...
    assert len(plan) == 21
    assert len({sample["sample_id"] for sample in plan}) == 21
    chunks = tuple(plan[index : index + len(DEFENSES)] for index in range(0, 21, 7))
...
def test_stable3_pilot_blocks_expand_to_balanced_147_sample_contract(
    planning_base: Campaign,
) -> None:
    assert tuple(sorted(CAMPAIGN_DIR.glob("classifier-stable3-pilot-*.yml"))) == PILOT_PATHS
    ...
    assert len({tuple(value["workloads"]) for value in values}) == 6
    ...
    assert len(combined) == 147
    assert len({sample["sample_id"] for sample in combined}) == 147
    ...
    workload_positions = Counter(
        (workload_id, position)
        for order in WORKLOAD_ORDERS
        for position, workload_id in enumerate(order)
    )
    assert Counter(workload_positions.values()) == Counter({2: 6, 3: 3})
    assert all(
        WORKLOAD_ORDERS[5].index(workload_id) != WORKLOAD_ORDERS[6].index(workload_id)
    ...
    assert max(defense_positions.values()) <= 2
