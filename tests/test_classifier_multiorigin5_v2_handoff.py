...
from qcsd_lab.util import atomic_text, load_json
from tools import classifier_handoff
from tests.test_classifier_multiorigin5_handoff import (
    _formal_export_metadata as _v1_formal_export_metadata,
)
from tests.test_classifier_handoff import _make_result, _rewrite_handoff_checksums
...
def _qualification_runtime(monkeypatch: pytest.MonkeyPatch) -> Iterator[None]:
    qualification_root = (
        ...
        / classifier_handoff.MULTIORIGIN5_V2_QUALIFICATION_SET
    )
    sidecars = [
        load_json(qualification_root / f"{workload_id}.json")
        for workload_id in classifier_handoff.MULTIORIGIN5_V2_CLASSES
    ]
    ...
    classifier_handoff._cached_multiorigin5_v2_configuration.cache_clear()
    yield
    classifier_handoff._cached_multiorigin5_v2_configuration.cache_clear()
...
def _receipts() -> list[SimpleNamespace]:
    ...
    for result_index, (name, campaign_file) in enumerate(
        zip(
            classifier_handoff.MULTIORIGIN5_V2_RESULT_NAMES,
            classifier_handoff.MULTIORIGIN5_V2_CAMPAIGN_FILES,
            strict=True,
        )
    ):
        ...
        campaign, configuration = classifier_handoff._expected_multiorigin5_v2_configuration(
            campaign_path
        )
...
def _rehearsal_receipt() -> SimpleNamespace:
    campaign_path = (
        ROOT / "config/campaigns" / classifier_handoff.MULTIORIGIN5_V2_REHEARSAL_CAMPAIGN_FILE
    )
    campaign, configuration = classifier_handoff._expected_multiorigin5_v2_configuration(
        campaign_path
    )
    ...
    return SimpleNamespace(
        experiment={
            "name": classifier_handoff.MULTIORIGIN5_V2_REHEARSAL_RESULT_NAME,
            "purpose": "evaluation",
            "source": dict(SOURCE),
            "samples": samples,
            "execution_order": [sample["sample_id"] for sample in samples],
...
def _formal_export_metadata() -> tuple[dict, list[dict]]:
    blocks = []
    rows = []
    for result_index, name in enumerate(classifier_handoff.MULTIORIGIN5_V2_RESULT_NAMES):
        acquisition_block = result_index // 2
        temporal_split = classifier_handoff.MULTIORIGIN5_V2_TEMPORAL_SPLITS[acquisition_block]
        baseline_result = result_index % 2 == 0
        policy = (
        ...
        defenses = (
            ("undefended",) if baseline_result else classifier_handoff.MULTIORIGIN5_V2_DEFENSES
        )
        visits = range(20) if baseline_result else range(10)
        for workload_id in classifier_handoff.MULTIORIGIN5_V2_CLASSES:
            for defense in defenses:
                runtime_kind, baseline = classifier_handoff.MULTIORIGIN5_V2_RUNTIME[defense]
                for visit in visits:
                    rows.append(
                        {
                            ...
                            "class_label": (
                                classifier_handoff.MULTIORIGIN5_V2_CLASS_LABELS[workload_id]
                            ),
                            "defense": defense,
        ...
        rows.append(
            {
                ...
                "workload_id": workload_id,
                "class_label": classifier_handoff.MULTIORIGIN5_V2_CLASS_LABELS[workload_id],
    ...
    return (
        {
            ...
            "blocks": [
                {
                    "result_name": classifier_handoff.MULTIORIGIN5_V2_REHEARSAL_RESULT_NAME,
                    "split": "interface",
                    "sample_splits": {
                        defense: "interface"
                        for defense in classifier_handoff.MULTIORIGIN5_V2_DEFENSES
                    },
                    "acquisition_block_index": 0,
                    "acquisition_block_id": "acquisition-block-001",
                    "source": dict(SOURCE),
...
def test_multiorigin5_v2_contract_freezes_exact_names_order_classes_and_usage() -> None:
    assert classifier_handoff.MULTIORIGIN5_V2_RESULT_NAMES == tuple(
        name
        for block in range(1, 11)
        for name in (
            f"research-classifier-multiorigin5-v2-baseline-{block:02d}-1200",
            f"research-classifier-multiorigin5-v2-paired-{block:02d}-1200",
        )
    )
    assert classifier_handoff.MULTIORIGIN5_V2_CAMPAIGN_FILES == tuple(
        name
        for block in range(1, 11)
        for name in (
            f"classifier-multiorigin5-v2-baseline-{block:02d}.yml",
            f"classifier-multiorigin5-v2-paired-{block:02d}.yml",
        )
    )
    assert classifier_handoff.MULTIORIGIN5_V2_CLASS_LABELS == {
    ...
    assert "the classifier-multiorigin5-v2 contract" in usage
    assert "baseline-01,\npaired-01" in usage


def test_multiorigin5_v2_collection_assigns_protocol_and_exact_qualification_binding() -> None:
    receipts = _receipts()

    plan = classifier_handoff._validate_pilot_collection(receipts, [None] * 20)

    assert isinstance(plan, classifier_handoff.Multiorigin5V2CollectionPlan)
...
@pytest.mark.parametrize(
    "campaign_file",
    [
        "classifier-multiorigin5-v2-baseline-01.yml",
        "classifier-multiorigin5-v2-paired-01.yml",
        "classifier-multiorigin5-v2-rehearsal.yml",
    ],
)
def test_multiorigin5_v2_expected_configuration_matches_materialized_frozen_reload(
    tmp_path: Path,
    campaign_file: str,
) -> None:
    campaign_path = ROOT / "config/campaigns" / campaign_file
    campaign, expected = classifier_handoff._expected_multiorigin5_v2_configuration(campaign_path)
    ...
    qualification_set = (
        None
        if "baseline" in campaign_file
        else classifier_handoff.MULTIORIGIN5_V2_QUALIFICATION_SET
    )
...
def test_multiorigin5_v2_collection_rejects_qualification_set_presence_or_value_drift(
    result_index: int,
) -> None:
    receipts = _receipts()
    configuration = receipts[result_index].experiment["configuration"]
    if result_index == 0:
        configuration["chaff_qualification_set"] = (
            classifier_handoff.MULTIORIGIN5_V2_QUALIFICATION_SET
        )
    else:
...
def test_multiorigin5_v2_rejects_noncanonical_or_mixed_result_lineages() -> None:
    receipts = _receipts()
    ...
    with pytest.raises(ValueError, match="cannot mix POC5, v1, and v2"):
        classifier_handoff._validate_pilot_collection(mixed, [None, None])

    mixed_v1_v2 = [
        _receipts()[0],
        SimpleNamespace(experiment={"name": classifier_handoff.MULTIORIGIN5_RESULT_NAMES[0]}),
    ]
    with pytest.raises(ValueError, match="cannot mix POC5, v1, and v2"):
        classifier_handoff._validate_pilot_collection(mixed_v1_v2, [None, None])
...
def test_multiorigin5_v2_formal_handoff_freezes_schema_v2_classifier_contract() -> None:
    dataset, rows = _formal_export_metadata()

    classifier_handoff._validate_multiorigin5_v2_handoff_protocol(dataset, rows)

    assert dataset["sample_count"] == 2_500
    assert dataset["counts_by_class"] == {
        label: 500 for label in sorted(classifier_handoff.MULTIORIGIN5_V2_CLASS_LABELS.values())
    }
...
def test_multiorigin5_v2_rehearsal_is_a_separate_30_sample_interface_gate() -> None:
    receipt = _rehearsal_receipt()
    plan = classifier_handoff._validate_pilot_collection([receipt], ["interface"])
    dataset, rows = _rehearsal_export_metadata()

    classifier_handoff._validate_multiorigin5_v2_rehearsal_handoff_protocol(dataset, rows)
...
def test_historical_multiorigin5_v1_protocol_still_validates_unchanged() -> None:
    dataset, rows = _v1_formal_export_metadata()

    classifier_handoff._validate_multiorigin5_handoff_protocol(dataset, rows)

    assert classifier_handoff.MULTIORIGIN5_COHORT == "classifier-multiorigin5-v1"
    assert dataset["sample_count"] == 2_500
...
@pytest.mark.parametrize(
    "result_name",
    [
        classifier_handoff.POC5_RESULT_NAMES[0],
        classifier_handoff.POC5_REHEARSAL_RESULT_NAME,
        classifier_handoff.MULTIORIGIN5_RESULT_NAMES[0],
        classifier_handoff.MULTIORIGIN5_REHEARSAL_RESULT_NAME,
        classifier_handoff.MULTIORIGIN5_V2_RESULT_NAMES[0],
        classifier_handoff.MULTIORIGIN5_V2_REHEARSAL_RESULT_NAME,
    ],
)
def test_full_validator_rejects_schema_v2_lineage_downgrade(
    tmp_path: Path,
    result_name: str,
) -> None:
    source, _experiment = _make_result(tmp_path)
    handoff = tmp_path / "handoff"
    classifier_handoff.export_classifier_handoff([source], handoff)
    dataset_path = handoff / "dataset.json"
    dataset = load_json(dataset_path)
    assert dataset["schema_version"] == classifier_handoff.SCHEMA_VERSION
    [block] = dataset["blocks"]
    block["result_name"] = result_name
    block["source_result"] = f"{result_name}/{block['run_id']}"
    atomic_text(dataset_path, json.dumps(dataset, indent=2, sort_keys=True) + "\n")
    _rewrite_handoff_checksums(handoff)

    with pytest.raises(ValueError, match="schema-v2 result lineage requires schema version 2"):
        classifier_handoff.validate_classifier_handoff(handoff)
