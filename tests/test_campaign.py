...
def test_completed_candidate_binding_requires_current_wakeup_schema(
    ...
    def terminal_receipts_valid(
        ...
        *,
        ...
        return (require_current_schema and schema == 9) or (
            not require_current_schema and 1 <= schema <= 8
        )
    ...
    monkeypatch.setattr(
        ...
        lambda value: value.get("schema_version") in {2, 5, 6, 7, 8, 9},
    )
    monkeypatch.setattr(
        orchestrator.capture_engine,
        "_validate_chaff_response_receipts",
    ...
    run["runner_wakeup_metrics"]["schema_version"] = 9
    _validate_run_binding(
        run,
        manifest=manifest,
        chaff_manifest=chaff_manifest,
